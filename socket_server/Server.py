    ...
    def backup_server_connection_handler(self):
        ...
        while backup_server_control:
            try:
                ...
                print(
                    '[**] Exception :: Back-up server connection accept :: ' + str(e))
...
class LockDevice:
    ...
    def print_details(self):
        print('\n')
        print('\n\n[*] LOCK >> username ' + str(self.username) +
              ' IP ' + str(self.ip) + ' Initilazing done.')
        print('\n')
...
def update_server_config(host):
    try:
        ...
        resp = urllib.request.urlopen(
            'https://techcodebox.000webhostapp.com/lock/server_ip.php?info=' + host)
...
backup = threading.Thread(target=BackupServer, args=(
    host, backupPort), name="backupThread")
