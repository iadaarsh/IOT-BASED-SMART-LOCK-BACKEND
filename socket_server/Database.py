...
class Database:
    ...
    def insert_online_device(self, username):
        try:
            sql = "INSERT INTO `onlinelist` (`id`, `username`) VALUES (NULL, '%s');" % (
                username,)
            self.cursor.execute(sql)
        except Exception as e:
    ...
    def insert_image_backup(self, username, path):
        try:
            ...
            sql = "INSERT INTO `imagebackup` (`id`, `username`, `path`) VALUES (NULL, '%s', '%s');" % (
                username, path)
            self.cursor.execute(sql)
            # auto commit
    ...
    def check_androidid_id(self, email, android_id):
        try:
            sql = "SELECT id FROM `owner` WHERE android_id = '%s' AND email = '%s'" % (
                android_id, email)
            self.cursor.execute(sql)
            result = self.cursor.fetchall()
            return result[0][0]
    ...
    def check_lock_mac(self, username, lock_mac):
        try:
            sql = "SELECT id FROM device WHERE username  = '%s' AND lock_mac = '%s'" % (
                username, lock_mac)
            self.cursor.execute(sql)
            result = self.cursor.fetchall()
            return result[0][0]
